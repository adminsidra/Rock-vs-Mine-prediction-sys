...
├── Copy of sonar data.csv            # Sonar dataset used for training/validation
├── Copy_of_Rock_vs_Mine_Prediction.ipynb  # Jupyter Notebook containing model exploration & training tests
...
2. If the model file is not found, the application automatically loads `Copy of sonar data.csv`, trains a **Logistic Regression** model, saves the trained model to `sonar_model.pkl`, and uses it.
